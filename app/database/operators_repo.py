...
async def set_operator_monobank_token(operator_id: int, token_encrypted: str):
    ...
    ГОЛОВНИЙ рубіж проти цього — порядок викликів на боці хендлера
    (app/handlers/operator_billing.py::_verify_and_save_new_token): банк
    питають ДО того, як ця функція взагалі викликається, і викликають її
    лише на success-гілці. Це скидання verified_at — ДРУГИЙ, підстраховувальний
    рубіж на випадок, якщо колись з'явиться ще один виклик цієї функції з
    іншого місця, що не пройшов через ту саму дисципліну «спершу банк, потім
    запис». Не прибирати цей другий рубіж лише тому, що перший здається
    достатнім.
    """
    pool = await get_db_pool()
    async with pool.acquire() as conn:
