...
def compute_uah_settlement_amount(tariff_uah_start, tariff_uah_kwh, kwh, reserved_uah) -> Decimal:
    ...
    перевитрату понад hold НЕ намагаємось стягнути — банк finalize понад
    утримання відхиляє, ПІДТВЕРДЖЕНО ЖИВИМ СМОУКОМ 30.07.2026, раніше було
    консервативне припущення). ОДНА реалізація, спільна для
    on_stop_transaction-шляху (kwh відомий одразу після StopTransaction) і
    кроку 3 звірки (kwh читається з уже 'completed' сесії) — не дві копії
    тієї самої формули.
