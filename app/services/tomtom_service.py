...
# живого смоуку): один handle_location() — це 1 виклик search_stations_
# near() + до MAX_STATIONS_SHOWN викликів get_availability() (app/handlers/
# user.py, по одному на кожну TomTom-станцію у ФІНАЛЬНІЙ видачі після
# злиття з OCM/оператором і дедупу; MAX_STATIONS_SHOWN=5 станом на зараз)
# = до 6 звернень за один пошук водія. Тобто бюджет 2000 — це ~330 пошуків
# водіїв на добу, НЕ 2000. TOMTOM_SEARCH_LIMIT (розмір ОДНІЄЇ відповіді
# nearbySearch, піднятий до 10, щоб дедуп мав із чого вибирати) на цю
# арифметику НЕ впливає — то самий один HTTP-виклик незалежно від значення.
