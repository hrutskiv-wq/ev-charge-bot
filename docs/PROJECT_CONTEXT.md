...
→ `0010` → … → `0019`. Один head, гілок немає.
...
**18 розходжень** плюс 5 таблиць, яких Alembic не створює.
...
**У схемі Alembic немає жодного зовнішнього ключа на `users`** — ні з `payments`,
ні з `kw_transactions`, ні з `ocpi_cdrs`. Це закриває питання, яке в
...
**43 файли `test_*.py`** у корені (не в пакеті). Повний прогін із CI-плейсхолдерами
(`BOT_TOKEN`, `GEMINI_API_KEY`, `OCPI_SECRET_TOKEN`) і живим `DB_URL` — **897 passed**
(21.08.2026); без `DB_URL` live-тести пропускаються.
...
(Postgres 16 + **чотири** live-тести; два з них про схему —
`test_alembic_from_scratch_live.py` і `test_schema_parity_live.py`, §7).
...
4. Разові скрипти з хардкодами `user_id` у корені (§12)
