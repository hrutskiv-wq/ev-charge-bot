...
`update_user_balance()` — `app/database/connection.py:208`. Гілки за `t_type`:

| гілка | рядок | поведінка |
|---|---|---|
| `deposit` / `monobank_jar` / `refund` | `:301` | кредит; рахунок **створюється**, якщо його немає |
| `release` | `:328` | кредит, ledger-тип `release`; теж у списку автостворення (§14) |
| `correction` | `:346` | **єдина знакова гілка** — знак приходить ззовні. Рахунку не створює |
| `hold` | `:377` | дебет із `WHERE balance >= $1`; **повертає `False`**, якщо не вистачило, і нічого не пише |
| інше (напр. `ocpi_session`) | `:405` | дебет **без** захисту від мінуса для наявного рахунку |

**Відсутній рахунок на дебетних гілках і на `correction` → `BalanceRowMissing`**
(`connection.py:13`), транзакція відкочується, журнал не поповнюється. До
21.08.2026 автостворення стояло перед усіма гілками, тому дебет неіснуючому
користувачеві створював рядок і заганяв баланс у мінус.

Знак у журналі: депозит `+`, списання `−`, коригування — як передали.

**Інваріант `SUM(kw_transactions.amount) = users.balance` тримається лише для
записів через цю функцію.** Абсолютне формулювання, яке тут стояло раніше,
хибне: запис повз неї вона не бачить. Саме так 15.07.2026 у прод потрапив
рядок журналу без рахунку — його писав вебхук Monobank
(`app/api/payments.py`, видалений 28.07.2026). Незалежно від шляху запису
цілісність `user_id` тепер тримає зовнішній ключ (міграція 0020, §7);
розбір — `docs/plan-orphan-balance-guard.md`.
...
**Alembic — джерело правди схеми.** Ланцюг: `b1b193e2bd7b` → `0007` → `0008` → `0009`
→ `0010` → … → `0020`. Один head, гілок немає.

**З 21.08.2026 це твердження перевіряється, а не декларується.** До того дня воно було
неправдою: `users` і `stations` не створювала жодна міграція, тому `alembic upgrade head`
на порожній базі падав на `0012_wallet_topups` (`REFERENCES users(user_id)`). Робоча база
виникала тільки як побічний ефект старту застосунку. Закрито міграцією
`0009_users_and_stations` (слот `0009` до того був порожній) і регресійним тестом
`test_alembic_from_scratch_live.py`, який піднімає окрему тимчасову базу, накочує
ланцюг і перевіряє FK-зв'язок до `users` позитивно й негативно. Тест доданий у
job `live-db-tests`; перевірено, що без міграції 0009 він падає.
...
1. **`payment_provider` ENUM не збігається.** Alembic (`b1b193e2bd7b:25`):
   ...
   стався з `refund` і був закритий міграцією 0008.** **Закрито 06.08.2026**
   міграцією 0019 і `ALTER TYPE` на проді (§14). Асиметрія лишилась у другий бік:
   `liqpay` є тільки в Alembic-гілці — див. таблицю нижче
2. ~~**`users` і `stations` створює тільки бутстрап**~~ — **закрито 21.08.2026**
   міграцією `0009_users_and_stations`. DDL скопійовано з бутстрапу дослівно,
   щоб не додати новий рядок розходження; `downgrade()` навмисно порожній —
   зворотна дія до створення `users` знищила б усі баланси разом із
   `kw_transactions` по каскаду
...
### Повний перелік тепер вимірюється, а не ведеться руками

**Перелік вище був неповним.** 21.08.2026 схеми звірено машиною —
`test_schema_parity_live.py` піднімає дві порожні бази (одну через
`alembic upgrade head`, другу через бутстрап) і порівнює
`information_schema.columns`, зовнішні ключі та `pg_enum`. Результат:
**17 розходжень** плюс 5 таблиць, яких Alembic не створює. Було 18 — рядок
`kw_transactions.user_id -> users.user_id` прибрано разом із міграцією 0020,
тим самим комітом (інакше тест паритету червоний в один бік або в інший).

Живий перелік — константа `KNOWN_DIFFERENCES` у самому тесті, з поясненням
кожного рядка. Тут він **не дублюється навмисно**: два переліки розійшлись би
рівно так, як розійшлись дві схеми. Нижче — тільки те, що варто знати, не
відкриваючи тест.

**У схемі Alembic до 21.08.2026 не було жодного зовнішнього ключа на `users`** —
ні з `payments`, ні з `kw_transactions`, ні з `ocpi_cdrs`. Міграція 0020 закрила
`kw_transactions`; `payments` і `ocpi_cdrs` лишаються в базлайні (§14). Це закриває питання, яке в
`CLAUDE.md` §6a лишалось відкритим: FK `ocpi_cdrs.user_id → users(user_id)`
на проді «не діє» **не тому, що зіпсувався**, а тому що таблиця приїхала
з початкової міграції, де ключа ніколи й не було. Прод відповідає
Alembic-гілці, і саме тому там колись з'явився CDR на `user_id = 12345`,
якого немає в `users`.

**Розходження зустрічне, а не одностороннє.** 0019 додав `telegram` в
Alembic-гілку, а `liqpay` лишився тільки в ній. Тобто це не «бутстрап
відстав», а дві схеми, що розповзлись в обидва боки.

**Індекси не порівнюються** — імена в двох механізмах різні за побудовою,
порівняння за іменем дало б шум. Не вважати їх покритими.
...
**44 файли `test_*.py`** у корені (не в пакеті). Повний прогін — **906 passed**
(22.08.2026, `main` = `4aa76fc`). Без `DB_URL` live-тести пропускаються; без трьох
змінних оточення сюїта взагалі не збирається (§14.7), тому команда фіксується
цілком:

```bash
docker compose up -d postgres
BOT_TOKEN="123456:ci-placeholder-bot-token" GEMINI_API_KEY="ci-placeholder-gemini-key" OCPI_SECRET_TOKEN="ci-placeholder-token" DB_URL="postgresql://ev_admin:CHANGE_ME@127.0.0.1:5432/ev_charge_base" python -m pytest -q
```

Значення трьох перших — дослівно з `ci.yml`, job `live-db-tests`. `DB_URL`
збирати **явно на `127.0.0.1`**, а не брати з `.env`: там може стояти прод,
а live-тести виконують `CREATE DATABASE`. На Windows додатково
`PYTHONIOENCODING=utf-8`.
`conftest.py` виставляє `OCPI_SECRET_TOKEN` до імпортів — **але не `BOT_TOKEN`
і не `GEMINI_API_KEY`**, тому з чистого термінала сюїта не збирається (§14.7). `pytest.ini`: `asyncio_mode=auto`,
...
(Postgres 16 + **п'ять** live-тестів: `test_start_ocpp_transaction_live.py`,
`test_charging_reservations_live.py`, `test_alembic_from_scratch_live.py`,
`test_schema_parity_live.py`, `test_orphan_balance_guard_live.py`).
Три останні — про схему й грошовий інваріант, і всі три перевірені на злам:
кожен доведено падає, якщо прибрати те, що він захищає.
...
`app/handlers/charge.py`, разові скрипти в корені. Виняток —
`scripts/repair_orphan_balance.py`: обидва його режими (дефолт і `--zero-out`)
покриті в `test_orphan_balance_guard_live.py`.

---

## 11. OCPI — вимкнено 06.08.2026

Роутери відключені: `app/main.py:20-21` (імпорти), `include_router(api_cdr_router)`
і `dp.include_router(bot_stations_router)` закоментовані. Код, таблиці й
`init_ocpi_tables()` лишаються. Детальний розбір — `CLAUDE.md` §6a.

Підстава: на проді `ocpi_cdrs` містить три рядки, і всі три — власні тести
(`cdr_prod_test_001`, `cdr_final_success_888`, `CDR-TEST-001`), кожен списав
`total_cost` замість `total_energy`, тобто прилетів до виправлення 17.07.
Решта OCPI-таблиць порожні, `OCPI_CPO_BASE_URL` не заданий, у логах за 30 діб тиша.
Партнера-CPO ніколи не існувало.

До вимкнення `POST /ocpi/emsp/2.2.1/cdrs` викликав `update_user_balance()` — тобто
це був відкритий грошовий ендпоінт без легітимного споживача, захищений одним
статичним токеном.

**Контроль:** `POST` на цей шлях віддає **405** (`StaticFiles` на `/` приймає лише
GET/HEAD). 401 означав би, що роутер знову підключений.

**Побічно:** `OCPI_SECRET_TOKEN` більше не валить старт, бо `OCPIConfig` не
завантажується. З `.env` не прибирати — `conftest.py` його виставляє.
...
2. **Звірки не мають планувальника** — механізм самозцілення є, викликати нікому (§6.5)
3. **Заглушка авторизації OCPP.** `on_authorize` завжди `Accepted`
   (`ocpp_ws.py:192`), а `StopTransaction` без резервації йде в гілку
   `complete_ocpp_transaction()` без грошей (`ocpp_ws.py:312`). Тобто транзакція,
   ініційована **на самій станції**, а не нашим RemoteStart, — безкоштовна
   електрика коштом оператора. Зараз недосяжно: підключених станцій немає.
   **Закривати до першої, а не після** — потім та сама зміна стає узгодженням
   із чужими конфігураціями (`AuthorizeRemoteTxRequests`, локальні списки, free-vend)
4. **У `payments` немає ознаки тестового платежу — і це вже коштує грошей у звіті.**
   Тестовий і бойовий записи виглядають однаково (`status='success'`). Рядок на
   750 грн від 15.07.2026, приписаний **ID самого бота** (8855895437), лишається
   в таблиці: FK на `payments.user_id` у гілці Alembic немає, тож ані ремонт, ані
   0020 його не зачіпають. Генератор звіту оператору й будь-яка звірка порахують
   ці 750 грн як виторг. **Має бути закрито ДО першого рахунку оператору** (5% з
   обороту або абонплата) — інакше перший же рахунок піде із завищеною сумою,
   і виявить це клієнт, а не ми.
5. **Гілка `release` стоїть у списку автостворення рахунку, хоча не мала б.**
   `release` — повернення раніше зарезервованого, тобто рахунок за визначенням
   уже існує. Оскільки `hold` без рахунку тепер падає `BalanceRowMissing`,
   `release` для неіснуючого рахунку **недосяжний** — це мертвий код, який у
   «неможливому» випадку мовчки вигадає кредит. Розглянути, чи має `release`
   теж падати (§6.1).
6. **FK `kw_transactions.user_id` має `ON DELETE CASCADE` — видалення користувача
   мовчки зітре його грошовий журнал.** Це суперечить принципу append-only,
   закріпленому після видалення рядків 16-23 у серпні 2026 (`CLAUDE.md` §6b).
   Вибір у 0020 свідомий — дослівно як у бутстрапі, щоб не створити нове
   розходження замість закритого, — але питання лишається відкритим:
   `ON DELETE RESTRICT` захистив би журнал, ціною зміни **обох** схем одночасно.
7. **`conftest.py` виставляє `setdefault` лише для `OCPI_SECRET_TOKEN`.**
   `BOT_TOKEN` і `GEMINI_API_KEY` — ні, тому з чистого термінала сюїта **не
   збирається**: 11 файлів падають на імпорті з `TokenValidationError`, і кожна
   нова сесія відкриває це заново. Додати два `setdefault` з плейсхолдерами того
   ж штибу, що в `ci.yml`. Це вже код — окремим бандлом; тут лише зафіксовано.
8. Разові скрипти з хардкодами `user_id` у корені (§12)

**Закрито 06.08.2026:** OCPI вимкнено (§11); `payment_provider` виправлено
міграцією 0019 і `ALTER TYPE` на проді; інваріант `SUM(kw_transactions) = users.balance`
відновлено коригувальною транзакцією на акаунті розробника (розбір — `CLAUDE.md` §6b).

**Закрито 21-22.08.2026 (бандл `orphan-balance-guard`):** дебет неіснуючому
користувачеві падає `BalanceRowMissing` замість того, щоб створювати рахунок із
відʼємним балансом (§6.1); додано гілку `correction`, якої бракувало з 06.08;
осиротілий рядок журналу розвʼязано штатно
(`scripts/repair_orphan_balance.py`, журнал не редагувався); FK
`kw_transactions.user_id -> users` накочено міграцією 0020. Розбір —
`docs/plan-orphan-balance-guard.md`. **На прод не деплоєно** станом на 22.08.2026:
база на 0019, ремонт не запускався.

**Закрито 21.08.2026:** `users`/`stations` тепер створює Alembic (міграція 0009),
чиста база з міграцій працездатна — `test_alembic_from_scratch_live.py`.
Розходження Alembic ↔ бутстрап більше не ведеться руками:
`test_schema_parity_live.py` порівнює дві реально підняті схеми й падає на
**новому** розходженні (§7). Обидва тести перевірені на злам: без міграції 0009
і при розведенні DDL на один символ вони червоні.
