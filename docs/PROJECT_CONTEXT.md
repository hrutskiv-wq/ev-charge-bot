...
2. **Немає тесту, що порівнює схему Alembic зі схемою бутстрапу.** Розходження
   вистрелило вже тричі: `refund` (0008), відсутній FK `ocpi_cdrs.user_id → users`
   (спостережено на проді 06.08), `payment_provider` без `telegram` (0019).
   Правило в `CLAUDE.md` §3 не рятує — рятує тест на `pg_enum` і `information_schema`
3. **Звірки не мають планувальника** — механізм самозцілення є, викликати нікому (§6.5)
4. `users`/`stations` не створюються Alembic — чиста база з міграцій непрацездатна (§7.2)
5. Разові скрипти з хардкодами `user_id` у корені (§12)
