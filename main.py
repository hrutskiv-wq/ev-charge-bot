...
# --- ПРІОРИТЕТНІ ОБРОБНИКИ ТЕКСТУ (ЙДУТЬ ПЕРШИМИ) ---
...
async def process_charge_click(message: types.Message):
    ...
    if balance <= 0:
        ...
        await message.answer(
            ...
            "• Надішліть геопозицію, і бот знайде найближчу реальну станцію.\n"
...
# --- Обробка локації та інлайн-тарифів ---
@dp.message(F.location)
async def handle_location(message: types.Message, state: FSMContext):
    await message.answer("🔍 **Звертаємось до бази даних Open Charge Map через інтернет... Обчислюємо відстань...**")
    station_data = await find_nearest_station(message.location.latitude, message.location.longitude)
    ...
    if station_data:
        ...
        await message.answer("❌ Станцій поблизу не знайдено в базі даних OCM.")
...
# --- Стан очікування технічних ID та Ваучерів ---
...
async def process_station_id(message: types.Message, state: FSMContext):
    ...
    if station_info:
        ...
        await message.answer("❌ Станцію з таким ID не знайдено в локальній базі.", reply_markup=get_main_menu())
...
async def handle_voice(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Перенаправляємо розпізнаний текст у відповідні функції
...
# --- УНІВЕРСАЛЬНИЙ ШІ-ЧАТ (НАЙПРАВІШИЙ, СТОЇТЬ ОСТАННІМ) ---
...
async def main():
    ...
    print("Database is verified. Architecture is correct. Running polling...")
